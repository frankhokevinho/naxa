...
if ($error == 0){
$res =  $db->query("UPDATE general_setting SET sitetitle='".$title."', colorcode='".$color."', currency='".$currency."', cur='".$cur."', reg='".$reg."', ev='".$ev."', mv='".$mv."', deci='".$deci."', en='".$en."', mn='".$mn."' WHERE id='1'");
if($res){
...
$old = $db->query("SELECT sitetitle, colorcode, currency, cur, reg, ev, mv, en, mn, deci FROM general_setting WHERE id='1'")->fetch();
...
<div class="col-md-12">
<input type="text" name="deci" class="form-control" value="<?php echo $old[9]; ?>">
</div>
</div>
</div>
